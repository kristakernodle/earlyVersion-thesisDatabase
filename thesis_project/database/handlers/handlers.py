...
def handler_seed_trials_table(postgresql):
    ...
    with TestingCursor(postgresql) as cursor:
        cursor.execute(
            "SELECT (session.mouse_id, session.experiment_id, session.session_date folders.folder_id, folders.folder_dir) "
            "FROM folders INNER JOIN sessions")
        database.seed_tables.seed_tables.seed_blind_trials_table(cursor, experiment_id, folder_id, trial_date,
                                                                 trial_dir)
