...
Postgresql = tpg.PostgresqlFactory(cache_initialized_db=True,
                                   on_initialized=database.handlers.handlers.handler_create_blind_folders_table)
...
class TestNewBlindFolder(unittest.TestCase):
    ...
    def setUp(self):
        ...
        database.handlers.handlers.handler_seed_blind_folders_table(self.postgresql)
        with TestingCursor(self.postgresql) as cursor:
            all_folder_dir = models.folders.list_all_folder_dir(cursor)
            all_reviewer_ids = models.reviewers.list_all_reviewer_ids(cursor)
